	...
	drop table if exists benefit_analyze.wuhy_2018_itv;
	create table benefit_analyze.wuhy_2018_itv distribute by hash (serv_id) 
	as
	select 
	t1.serv_id
	,case when 1=1 then '04000004' end as group_id
	,t1.serv_grp_type 
	,t1.area 
	,t1.manage_area
	,case when 1=1 then '特殊群' end as user_type
	,case when 1=1 then 'itv4' end as user_type2
	,t1.charge_type
	,t1.pre_charge_2018
	,t1.avg_charge_3
	,t1.avg_charge_6
	...
	(select distinct serv_id from benefit_analyze.dcz_itv_prod_offer_201712  where prod_offer_code='DM0001-060_013' and msinfo_open_date>='2017-1-1')  c2
     ...
     where c1.serv_id is null and c2.serv_id is null and b6.acc_nbr is null
	;
	
	select * from benefit_analyze.wuhy_2018_itv;
	
	select count(serv_id), count(distinct serv_id) from benefit_analyze.wuhy_2018_itv;
