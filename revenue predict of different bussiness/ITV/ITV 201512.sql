	...
	------------------------------------2015年12月不存在2381 ITV用户---------------------------------------------
	select * from benefit_analyze.cust_infor_201711 where prod_id='2381';
	
	-----------------------------------验证异常用户的存在--------------------------------------
	
	select serv_id_m
	,(charge_公允后-(case when charge_公允前的一次性费用 is null then 0 else charge_公允前的一次性费用 end)) as charge_费用
	from benefit_analyze.ZYJ_QRSR_201512_res
	where prod_id in ('1004','3816','500000165','500000166') and (charge_公允后-(case when charge_公允前的一次性费用 is null then 0 else charge_公允前的一次性费用 end))>50000;
...
/*	,case when t1.serv_grp_type='01' then '政企'
	...
	,sum( t5.charge_费用) as charge_201612
